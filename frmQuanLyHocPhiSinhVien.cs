...
namespace Education_Manager
{
    public partial class frmQuanLyHocPhiSinhVien : Form
    {
        ...
        // Cấu hình các control trên form cho phù hợp logic mới
        private void SetupFormControls()
        {
            // Vô hiệu hóa các control không được phép chỉnh sửa
            txtMaSV.ReadOnly = true;
            cbHocKy.Enabled = false; // Dùng ComboBox không cho sửa
            numSoLuong.Enabled = false;
            numTongSoTien.Enabled = false;

            // Chỉ cho phép chỉnh sửa trạng thái
            cbTinhTrang.DropDownStyle = ComboBoxStyle.DropDownList;
        }

        // Tải dữ liệu từ CSDL, JOIN với bảng Sinh_vien để lấy họ tên
        private void LoadData()
        {
            ...
            try
            {
                ...
                // Cập nhật lại tên các cột cho DataGridView nếu cần
                // Ví dụ: dataGridView1.Columns["id_sinh_vien"].DataPropertyName = "id_sinh_vien";
        ...
        private void DataGridView1_SelectionChanged(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                // Hiển thị họ tên ở đâu đó nếu bạn có thêm Label
                cbHocKy.Text = row.Cells["hoc_ky"].Value?.ToString();
                numSoLuong.Value = Convert.ToDecimal(row.Cells["tong_so_tin_chi"].Value);
                numTongSoTien.Value = Convert.ToDecimal(row.Cells["tong_so_tien"].Value);
                cbTinhTrang.Text = row.Cells["trang_thai"].Value?.ToString();
            }
        }
        ...
        private void btnSua_Click(object sender, EventArgs e)
        {
            ...
            SqlParameter[] parameters = {
                new SqlParameter("@trang_thai", cbTinhTrang.Text),
